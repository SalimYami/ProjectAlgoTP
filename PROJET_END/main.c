...
void BoutonFini(button button0,int width,int height,char* chaine){
    ...
    if(mouseonbutton(button0)){
           if(chaine != "PROB MODIF") button0.col=GREEN;
           else{button0.col=GRAY; chaine="No Service";}
           if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && chaine=="No Service" && IsKeyPressed(KEY_LEFT_SHIFT))  chaine="VOIR";    
            ...
            if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && chaine=="VOIR")  Saisie=true;  
             if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && chaine=="RESULTAT") vraifin=true;                                                               
      ...
      if (IsMouseButtonDown(0))
       ...
       // rotation
        if (IsKeyPressed(KEY_LEFT)) // rotation a gauche
            ...
            cam.offset.x-=50;
            BeginMode2D(cam);
               DrawText("AVANT TRI",GetScreenWidth()/2-150,200,30,ORANGE); 
    DrawText("APRES TRI",GetScreenWidth()/2-150,400,30,GREEN); 
for(int j=0 ; j<n ; j++)
{   C[j].box.y=250;
    poscx= C[j].box.x;
    poscy=C[j].box.y;
    ...
    sizetext=20;          
        DrawRectangleRec(C[j].box,ORANGE); //une case 
        sprintf(text, "%d", C[j].val);
    DrawText(text,poscx + MeasureText(text,sizetext) ,poscy + sizec/2 - sizetext/2 ,sizetext, WHITE);
    T[j].box.y=450;
    T[j].box.x= C[j].box.x;
    poscy=T[j].box.y;
    sizec=T[j].box.height;
    sizetext=20;          
        DrawRectangleRec(T[j].box,GREEN); //une case 
        sprintf(text, "%d", T[j].val);
    DrawText(text,T[j].box.x + MeasureText(text,sizetext) ,T[j].box.y + sizec/2 - sizetext/2 ,sizetext, WHITE);

    } EndDrawing();
    DrawText("RETURN (SUPP)",GetScreenWidth()/2-150,screenHeight-100,30,RED);
    if(IsKeyPressed(KEY_BACKSPACE)){vraifin=false;}}
       if(Dessin){
            BeginDrawing();
            ClearBackground(new);
    ...
    if (((Key >= KEY_ZERO && Key <= KEY_NINE) || (Key>= KEY_KP_0 && Key<= KEY_KP_9 ) ) && (cpt < MAX_INPUT_CHARS))
     {
      ...
      Key=GetCharPressed();
        ...
        {   cpt--;
         ...
         if(IsKeyPressed(KEY_ENTER)) {interrupt=true; nbr=cve(textee);}
...
if(f>N){f=0; Text3=true; Affichage=true;}
if(cp>=N-f+1){cp=0; f=f*2; Affichage=true; repeter=false; WaitTime(1); }
if(repeter){cp+=f; Affichage=true; repeter=false; }
if(Text3){DrawText("SUPP POUR END LE PROGRAMME",GetScreenWidth()/2-150,GetScreenHeight()+200,30,GREEN);}
if(IsKeyPressed(KEY_BACKSPACE)){Text3=false; fin=true; Debut=false; cam.offset.y =GetScreenHeight() / 10.0f;}
...
//gcc -I src/include -L src/lib -o tableaucase TableauFinal.c -lraylib -lopengl32 -lgdi32 -lwinmm
